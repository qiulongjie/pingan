...
function checkForm(o) {
   ...
   if (document.getElementById('txtName').value == '') {
    ...
    if (jsGetAge($.trim($("#txtBirthday").val())) < 25) {
        alertText('年龄须在25-50周岁之间！');
        $('#txtBirthday').css({ 'borderColor': 'red' });
        document.getElementById('txtBirthday').value = '';
        document.getElementById('txtBirthday').focus();
		flag = false;
    ...
    if (flag) 
	{
		...
		if($.cookie("cookie"+a)){
			...
			if ($.cookie("cookie_cnt"+a)){
				...
				cnt = cnt - 1;
				$.cookie("cookie_cnt"+a,  cnt+"", {
        			expires : parseInt(p)
        		});
			}
		}else{
			$.cookie("cookie"+a, "true", {
    			expires : parseInt(p)
    		});
			var cnt = 1;
			if(parseInt(p2)){
